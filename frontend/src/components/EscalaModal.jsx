    ...
    <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center px-4 sm:px-6">
      ...
      <div className="relative z-10 w-full max-w-lg max-h-[92vh] bg-[#1A1A1C] border border-[#3D3D40] rounded-t-2xl sm:rounded-xl shadow-2xl flex flex-col">
        ...
        <div className="flex-1 overflow-y-auto px-6 py-4 space-y-4">
          {/* nome */}
          <div>
            <label className="block text-xs text-[#BFBFC3] mb-1">Nome da Escala *</label>
            <input
              value={form.nomeEscala}
              onChange={(e) => handle("nomeEscala", e.target.value)}
              placeholder="Ex: 5x2, 6x1, 12x36"
              className="w-full px-4 py-2.5 rounded-xl bg-[#2A2A2C] border border-[#3D3D40] text-white text-sm focus:outline-none focus:ring-1 focus:ring-[#FA4C00]"
            />
          </div>

          {/* tipo */}
          <div>
            <label className="block text-xs text-[#BFBFC3] mb-1">Tipo (opcional)</label>
            <input
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label className="block text-xs text-[#BFBFC3] mb-1">Dias Trabalhados</label>
              <input
                type="number"
                min="1"
            ...
            <div>
              <label className="block text-xs text-[#BFBFC3] mb-1">Dias de Folga</label>
              <input
                type="number"
                min="1"
          ...
          {/* descrição */}
          <div>
            <label className="block text-xs text-[#BFBFC3] mb-1">Descrição (opcional)</label>
            <textarea
              value={form.descricao}
              onChange={(e) => handle("descricao", e.target.value)}
              placeholder="Informações adicionais sobre a escala"
              rows={3}
              className="w-full px-4 py-2.5 rounded-xl bg-[#2A2A2C] border border-[#3D3D40] text-white text-sm focus:outline-none focus:ring-1 focus:ring-[#FA4C00] resize-none"
            />
          </div>
          ...
          <div>
            ...
            <select
              ...
              className="w-full px-4 py-2.5 rounded-xl bg-[#2A2A2C] border border-[#3D3D40] text-white text-sm"
            >
        ...
        {/* FOOTER */}
        <div className="flex flex-col sm:flex-row justify-end gap-3 px-6 py-4 border-t border-[#3D3D40]">
